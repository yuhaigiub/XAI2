...
class GraphWaveNet(nn.Module):
    ...
    `Graph WaveNet for Deep Spatial-Temporal Graph Modeling <https://arxiv.org/abs/1906.00121>`_ paper. 
    The model takes a list of node embeddings across several time steps and predicts the output embeddings for the specified number of upcoming time steps.
    ...
    Args:
        ...
        dilations (list(int), optional): 
            The set of dilations applied by each temporal convolution layer. 
            For each dilation :obj:`d`, the temporal convolution captures information from :obj:`d` time steps prior. 
            (default: :obj:`[1, 2, 1, 2, 1, 2, 1, 2]`)
        adaptive_embeddings (int, optional): The size of the embeddings used to calculate the adaptive matrix. (default: :obj:`10`)
    ...
    def forward(self, x, edge_index, edge_weight=None):
        ...
        Args:
            x (torch.Tensor): The temporal node features of shape :math:`(*, t_{input}, |\mathcal{V}|, F_{in})`
            edge_index (torch.Tensor): The edge indices of shape :math:`(2, |\mathcal{E}|)`
            edge_weight (torch.Tensor, optional): The edge weights shape of :math:`(|\mathcal{E}|)` (default: :obj:`None`)

        Return types:
            * **x** (*torch.Tensor*): Predicted temporal node features of shape :math:`(*, t_{output}, |\mathcal{V}|, F_{out})`
        """
        ...
        # If the dilation steps require a larger number of input time steps, padding is performed
        ...
        if is_batched:
            ...
            x = x.reshape(self.out_timesteps, self.out_channels, self.num_nodes).transpose(-1, -2)
